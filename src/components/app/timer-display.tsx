...
import { Play, Pause, RotateCcw, SkipForward, Edit, Plus, Trash2, Copy, Loader2 } from "lucide-react";
    ...
    if (title.trim() && !isNaN(newDuration) && newDuration >= 0.1) {
      ...
      onSave({
        ...
        soundFile: selectedSound
          ? { url: selectedSound.url, name: selectedSound.name, type: selectedSound.type || "" }
          : null,
      });
    }
  };
...
export function TimerDisplay() {
  ...
  const [isCloning, setIsCloning] = useState(false); // Loading khi clone
  ...
  // Đảm bảo cycle là editable trước khi thực hiện hành động
  const ensureEditable = async (action: () => void | Promise<void>): Promise<void> => {
    if (!currentCycle) return;

    // Nếu đã là private cycle → thực hiện luôn
    if (currentCycle.authorId === user?.uid) {
      if (typeof action === "function") action();
      return;
    }

    // Nếu là template → clone trước
    setIsCloning(true);
    try {
      const editableCycle = await makeCurrentCycleEditable();
      if (editableCycle && typeof action === "function") {
        await action();
      }
    } finally {
      setIsCloning(false);
  ...
  // Các handler phải là async + await
  const handleEditCycleName = async () => {
    await ensureEditable(() => setIsEditingCycleName(true));
  };

  const handleEditPhase = async (phaseId: string) => {
    await ensureEditable(() => setEditingPhaseId(phaseId));
  };

  const handleAddPhaseClick = async () => {
    await ensureEditable(() => setIsAddingPhase(true));
  };

  const handleDeletePhase = async (phaseId: string) => {
    await ensureEditable(async () => {
  ...
  // Copy Cycle (chỉ từ state, không cần Firestore)
  const handleCopyClick = async () => {
    if (!user) {
      showLoginPrompt("sao chép chu kỳ");
      return;
    }
    if (!currentCycle) return;
    ...
    try {
      ...
      toast({
        title: "Lỗi",
        description: "Không thể sao chép cycle. Vui lòng thử lại.",
        variant: "destructive",
      });
    }
  };

  const debouncedClone = useCallback(debounce(handleCopyClick, 500), [
    user,
    currentCycle,
    cloneCycle,
    showLoginPrompt,
        ...
        {isEditingCycleName ? (
          <div className="flex flex-col gap-2">
            <Input
              ...
              onBlur={(e) => handleSaveCycleName(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleSaveCycleName(e.currentTarget.value)}
              autoFocus
              className="text-3xl font-headline tracking-wider text-center"
            />
            <Button size="sm" onClick={() => setIsEditingCycleName(false)}>
              Done
            </Button>
          </div>
        ) : (
          <div className="flex items-center justify-center gap-2">
            ...
            <Button
              variant="ghost"
              size="icon"
              onClick={handleEditCycleName}
              disabled={isCloning}
              title="Edit Cycle Name"
            >
              {isCloning ? <Loader2 className="h-5 w-5 animate-spin" /> : <Edit className="h-5 w-5" />}
            </Button>
          </div>
        )}
      </CardHeader>
              ...
              <div key={phase.id} className="w-full">
                <div className="flex items-center gap-2 w-full">
                  ...
                  <Button
                    ...
                    title="Edit Phase"
                  >
                    {isCloning ? <Loader2 className="h-4 w-4 animate-spin" /> : <Edit className="h-4 w-4" />}
                  </Button>
                  {totalPhases > 1 && (
                    <Button
                      variant="ghost"
                    ...
                    >
                      {isCloning ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                    </Button>
                  )}
                </div>
            ...
            <div className="w-full max-w-sm mx-auto">
              <PhaseEditor
                ...
                phase={{
                  title: "New Phase",
                  duration: 5,
                  soundFile: audioLibrary[0]
                    ? {
                        url: audioLibrary[0].url,
                        name: audioLibrary[0].name,
                        type: audioLibrary[0].type || "",
                      }
                    : null,
                }}
        ...
        <div className="flex items-center justify-center gap-2 pt-4 border-t w-full max-w-sm mx-auto">
          ...
          >
            {isCloning ? (
              ...
              <>
                <Copy className="mr-2 h-4 w-4" /> Copy Cycle
              </>
            )}
          </Button>
        </div>
