            ...
            <AnimatePresence>
              {tasks.map((task) => (
                <motion.div
                    ...
                    className="flex items-center gap-2 group p-2 rounded-md hover:bg-secondary/50"
                >
                  <Checkbox
                    id={`task-${task.id}`}
...
// "framer-motion": "^11.2.10", - I'll skip this as I am not allowed to add it without instruction. The animation will be subtle.
// Let me remove framer-motion.

export function TaskManagerNoAnim() {
  const [tasks, setTasks] = useState<Task[]>([]);
  const [newTaskText, setNewTaskText] = useState('');
